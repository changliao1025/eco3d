...
//
#include "date.h"
date::date(const date &Orig) { lJulianDay = Orig.lJulianDay; }
...
date::date(const char *String) { lJulianDay = atol(String); }
...
long date::YmdToJd(const int iYear, const int iMonth, const int iDay)
{
  ...
  jul_day = (long)(365.25 * year - year_corr) + (long)(30.6001 * (month + 1)) +
            day + 1720995L + b;
#else
...
int date::DayOfWeek(void) const { return ((((int)(lJulianDay % 7L)) + 1) % 7); }
...
int date::DayOfWorkWeek(void) const { return ((int)(lJulianDay % 7L)); }
