...
std::string convert_double_to_string(int iPrecision_in, int iWidth_in,
                                     double dNumber_in)
{
  std::string str_out;
  std::stringstream ss;
  ss << std::fixed << std::setw(iWidth_in) << std::setprecision(iPrecision_in)
     << dNumber_in;
  str_out = ss.str();
  return str_out;
}
...
double convert_from_joule_per_meter_to_calorie_per_centimeter(
    double dJoule_per_meter_in)
{
...
double convert_from_calorie_per_centimeter_to_joule_per_meter(
    double dCalorie_per_centimeter_in)
{
  ...
  if (lLength > 0)
    {
      ...
      std::vector<std::string> vTokens_out(iterator_begin,
                                           iterator_end); // done!
      return vTokens_out;
    }
  else
