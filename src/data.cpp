...
float **data::read_binary(string sFilename_in, long lColumn_in, long lRow_in)
{
  float dummy;
...
// 50==================================================
int data::write_binary_vector(std::string sFilename_out,
                              std::vector<double> vData_in)
{
  int error_code = 1;
  ...
  std::ofstream ofs;
  for (iIterator_double = vData_in.begin(); iIterator_double != vData_in.end();
       iIterator_double++)
    {
  ...
  // 50==================================================
  iIterator_float = std::max_element(vData_float.begin(), vData_float.end());
  ...
  if (dDummy1 == -9999.00) // we define -9999 as the missing value
  ...
  else
    {
      ...
      if (ofs.good())
        {
          ofs.write(reinterpret_cast<char *>(&vData_float[0]),
                    vData_float.size() * sizeof(float));
          error_code = 1;
        }
      else
        {
